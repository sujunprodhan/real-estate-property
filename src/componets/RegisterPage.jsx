...
const RegisterPage = () => {
  ...
  const onSubmit = (data) => {
    setLoading(true);
    const formData = { ...data, avatar: avatarPreview };
    console.log('Register Form Data with Avatar:', formData);
    
    // Mimicking network requests
    setTimeout(() => {
      setLoading(false);
      alert('Account Created with Profile Image Successfully!');
    }, 1500);
  };
      ...
      <div className="w-full max-w-5xl bg-base-200/60 backdrop-blur-xl border border-base-300/50 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl p-4 sm:p-6 flex flex-col lg:flex-row gap-6 relative overflow-hidden transition-colors duration-300">
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            ...
            {/* Avatar / Profile Image Upload Dropzone */}
            <div className="flex flex-col items-center gap-1.5 pb-1">
              <label className="text-[10px] font-extrabold text-base-content/70 uppercase tracking-wider block">Profile Image</label>
              <div className="relative group cursor-pointer w-16 h-16">
                <div className="w-full h-full rounded-full border-2 border-dashed border-base-300 hover:border-primary bg-base-100 flex items-center justify-center overflow-hidden transition-all shadow-xs">
                  {avatarPreview ? (
                    <Image width={64} height={64} src={avatarPreview} alt="Avatar Preview" className="w-full h-full object-cover" />
                  ) : (
                    <div className="flex flex-col items-center justify-center text-base-content/40 group-hover:text-primary transition-colors">
                      ...
                      <span className="text-[7px] font-extrabold mt-0.5 tracking-wider">UPLOAD</span>
                    </div>
                  )}
                </div>
                <input 
            ...
            {/* Full Name Input */}
            <div className="space-y-1">
              <label className="text-[10px] font-extrabold text-base-content/70 pl-2 uppercase tracking-wider block">Full Name</label>
              <input 
                type="text"
              ...
              />
              {errors.name && (
                <span className="text-[9px] font-bold text-error pl-3 block">{errors.name.message}</span>
              )}
            </div>
            ...
            {/* Password Input */}
            <div className="space-y-1">
              <label className="text-[10px] font-extrabold text-base-content/70 pl-2 uppercase tracking-wider block">Password</label>
